...
### Obločno Varjenje (MMA - Manual Metal Arc / Stick Welding)

**Značilnosti**:

Uporablja električni tok za ustvarjanje obloka med elektrodo in kovino.
Elektroda se tali in tvori varilni spoj. Plašč elektrode ob tem izgori in zagotavlja naslednje funkcije:

- **Zaščita zvara** - Tvori zaščitno atmosfero pred oksidacijo in kontaminacijo (kalcijev karbonat $CaCO_3$ ustvarja veliko $CO_2$),
- **Stabilizacija obloka** - Omogoča enakomeren prenos električnega toka (rutilne elektrode s $TiO_2$),
- **Oblikovanje žlindre** - Ustvari žlindro, ki varuje strjevanje zvara (silikati in aluminati),
- **Legiranje zvara** - Doda zlitine za izboljšanje mehanskih lastnosti (krom, nikelj, mangan).
...
1. Začetek taljenja: Ko se plazma ustvari, začne taliti površino osnovnega materiala in kovinsko žico. Kovinska žica se tali in kapljice staljene kovine padajo v talilni bazen.
...
- Povratni kabel (masa): Povratni kabel priključite neposredno na kos kovin, ki ga boste varili. Zagotovite dober električni stik. Vsak nezadosten stik površine bo povzročil iskrenje in s tem trošenje toplote.
- Povratni kabel priključite na varilni aparat na negativni pol (pri MIG/MAG varjenju je to skoraj pravilo).
...
- Priključite varilno pištolo z vodom za plin in žico na varilni aparat. Polariteta elektrode naj bo priključena na pozitivni pol varilnega aparata (na tem mestu se ustvari nekoliko več toplote - ioni in elektroni dejansko trkajo v elektrodo).
...
- **Položaj pištole**: Držite varilno pištolo pod kotom približno 10-15 stopinj proti/v smeri varjenja. To pomaga pri boljšem nadzoru nad talilnim bazenom in izboljša penetracijo (začetnik naj daje prednost preglednosti pred naklonom var. pištole).
- **Odmik**: Odmik kontaktne šobe od talilnega bazena naj bo okoli 15 mm. Z odmikom vplivamo na velikost stožca obloka in s tem porazdelitev toplote. Večji ko je odmik, nižja bo temperatura varjenja (območje dovajanja toplote pa je večje). Pri večjem odmiku se podaljša tudi razdalja elektrode med kontaktoma in s tem se poveča upornost na elektrodi, ki povzroči padec napetosti in s tem manjši oblok.
...
- **V-kot:** Priporočljivo za srednje debele in debelejše kovine (2 - 10 mm), običajno med 45° do 60° za V-priprave (30° na vsak varjenec). To zagotavlja dober dostop do korena zvara in ustrezno penetracijo.
...
Seveda — tukaj je isto podpoglavje preoblikovano v bolj **priročniški (učbeniški, tekoč, nekoliko bolj praktično usmerjen) slog**:

## DEFORMACIJE PRI VARJENJU

Pri varjenju skoraj vedno pride do določenih deformacij materiala. Te so posledica neenakomernega segrevanja in ohlajanja, ki povzroča notranje napetosti v zvaru in okolici. Med varjenjem se material lokalno segreje na zelo visoke temperature, zaradi česar se razširi. Ker pa je segret le omejen del, okoliški hladnejši material preprečuje prosto raztezanje. V segretem območju zato nastanejo tlačne napetosti.

Ko se material po varjenju začne ohlajati, se skrči. Ta skrček pa ni enakomeren, saj je bil material med segrevanjem omejen. Posledica so trajne deformacije in preostale napetosti v materialu. Ključni vzrok deformacij je torej:
> **neenakomeren vnos toplote in omejeno raztezanje materiala**

Deformacije so izrazitejše, kadar:

* je vnos toplote velik (visok tok, počasno varjenje),
* je toplota koncentrirana na majhnem območju,
* varimo v eni smeri brez uravnoteženja,
* je material tanek (manj togosti),
* obdelovanec ni ustrezno vpet ali podprt.

### Kako zmanjšamo deformacije?

Deformacije pri varjenju zmanjšujemo predvsem z nadzorom vnosa toplote in z enakomerno porazdelitvijo napetosti v materialu. Več kot vnesemo toplote, večje bo raztezanje in kasnejši skrček, zato skušamo variti čim bolj “hladno”, vendar še vedno dovolj za kakovosten zvar. To v praksi pomeni višjo hitrost varjenja in ustrezno izbiro toka.

Pomemben vpliv ima tudi način varjenja. Če na primer varimo dolgo pločevino z enim neprekinjenim zvarom, se bo ta po ohlajanju pogosto ukrivila (“banana efekt”). To lahko zmanjšamo tako, da zvar razdelimo na krajše odseke in varimo izmenično (npr. najprej sredino, nato levo in desno). Podoben pristop uporabimo pri V-spoju, kjer se spoj med varjenjem rad zapira – zato lahko začetni kot nekoliko povečamo ali varimo simetrično na obeh straneh. Pri načrtovanju varjenja je smiselno predvideti, kje bodo deformacijski učinki največji, in ta mesta variti šele proti koncu postopka (na primer notranji rob okvirja). Prvi zvari imajo namreč največji vpliv na geometrijo konstrukcije, saj material še ni omejen z drugimi zvari in se lahko prosto deformira. Kasnejši zvari delujejo v že delno fiksirani strukturi, kjer so premiki omejeni. Zato se v praksi pogosto najprej izvede točkovno varjenje (tack welds), s katerim elemente začasno fiksiramo v pravilni legi. S tem zmanjšamo možnost premikov in omogočimo bolj kontrolirano izvajanje končnih zvarov na mestih, kjer bi sicer prišlo do največjih deformacij.

Pri tankih materialih, kot je 2 mm pločevina, se pogosto pojavi valovanje. V tem primeru pomagajo kratki, prekinjeni zvari in točkovno pritrjevanje (tack welds), ki ohranja obliko med varjenjem. Pri daljših profilih ali okvirjih pa lahko z izmeničnim varjenjem na nasprotnih straneh zmanjšamo krivljenje konstrukcije.

Uporaba vpenjalnih priprav dodatno omeji premikanje materiala, vendar moramo paziti, da ne ustvarimo prevelikih notranjih napetosti. Ključno je, da že pred začetkom razmislimo, kako se bo material krčil, in temu prilagodimo zaporedje varjenja.

Deformacije lahko v veliki meri omejimo že med samim varjenjem:

* zmanjšamo vnos toplote (nižji tok, hitrejši pomik),
* varimo **simetrično** (izmenično na obeh straneh),
* uporabimo **točkovne zvare (tack welds)** za fiksiranje,
* uporabimo **vpenjalne priprave**,
* izberemo pravilno **zaporedje varjenja** (npr. od sredine proti robovom),
* po potrebi uporabimo **predgrevanje ali kontrolirano ohlajanje**.

## NAJPOGOSTEJŠE NAPAKE PRI VARJENJU IN KAKO JIH ODPRAVITI

Varjenje je kompleksen proces, ki zahteva natančnost, znanje in prakso. Začetniki pogosto naredijo napake, ki lahko vplivajo na kakovost varilnega spoja ([The problem of welding](https://www.youtube.com/watch?v=p_0duihxmW4)). Tukaj so najbolj pogoste napake pri varjenju, njihove možne razlage in kako jih odpraviti:
...
[^Ocenjevanje]: Pripravite poročilo, ki vključuje:

- **Uvod:** Kratka predstavitev izbrane teme in ciljev naloge.
- **Metodologija:** Opis uporabljenih metod in orodij.
- **Rezultati:** Vključno s fotografijami zvarov, diagrami in opisi.
- **Razprava:** Analiza opazovanih razlik, ugotovljenih napak in njihovih posledic.
- **Zaključek:** Povzetek ugotovitev in lekcij, pridobljenih iz naloge.
- **Ocenjevanje**: Naloga bo ocenjena glede na naslednje kriterije:
  1. Natančnost in jasnost dokumentacije,
  2. Kakovost praktične izvedbe,
  3. Razumevanje in analiza izbranega parametra ali napake,
  4. Kreativnost in zmožnost kritičnega razmišljanja pri analizi rezultatov.
